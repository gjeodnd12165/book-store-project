...
-- 주문하기
-- 배송 정보 입력
INSERT INTO deliveries (address, receiver, contact) VALUES ("서울시 중구", "김송아", "010-1234-5678");

-- 주문 정보 입력
INSERT INTO orders (delivery_id, user_id) SELECT MAX(id), 1 FROM deliveries;
...
SELECT (SELECT MAX(id) FROM orders), 
  book_id, 
  quantity 
FROM cartItems WHERE cartItems.id IN (5, 1, 2);
